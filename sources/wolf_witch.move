...
module nft_war::wolf_witch {
    ...
    const MINIMUM_ELAPSED_TIME:u64 = 21600; // 4 times every one days. 
    ...
    const PRE_SEASON_WEREWOLF_AND_WITCH_COLLECTION:vector<u8> =b"WEREWOLF AND WITCH #S08"; // lose faction
    const WEREWOLF_AND_WITCH_COLLECTION:vector<u8> =b"WEREWOLF AND WITCH #S09";
    ...
    entry fun battle<CoinType, WarCoinType>(holder: &signer, 
        ...
        if(name_1 == strong_one) {
            if(random < win_rate) {
                ...
                item_material_drop(holder, game_address, string::utf8(MATERIAL_J), 10);
            } else {
                ...
                item_material_drop(holder, game_address, string::utf8(MATERIAL_C), 5);
            };
        } else {
            if(random < lose_rate) { 
                // let battle_field = borrow_global_mut<BatteArena>(game_address);            
                ...
                item_material_drop(holder, game_address, string::utf8(MATERIAL_I), 10);
            } else { 
                let token = token::withdraw_token(holder, token_id_1, 1);                        
                token::direct_deposit_with_opt_in(fighter.owner, token);
                event::emit_event(&mut game_events.game_result_events, GameResultEvent {            
    ...
    entry fun battle_with_bet<CoinType, WarCoinType>(holder: &signer, 
        ...
        if(name_1 == strong_one) { // if i'm strong
            ...
            if(random < win_rate) {
                ...
                item_material_drop(holder, game_address, string::utf8(MATERIAL_A), 20);
            } else {                
                ...
                item_material_drop(holder, game_address, string::utf8(MATERIAL_B), 10);
            };
        } else { // if i'm weak
            ...
            if(random < lose_rate) { 
                ...
                item_material_drop(holder, game_address, string::utf8(MATERIAL_C), 20);
            } else {                 
                let coins = coin::withdraw<WarCoinType>(holder, pay_amount);
                coin::deposit(fighter.owner, coins);
    ...
    entry fun bulk_mint<CoinType>(receiver: &signer, game_address:address, amount:u64) acquires WarGame, GameEvents, WhiteList {
        ...
        item_material_drop(receiver, game_address, string::utf8(MATERIAL_E), 5);
        while (ind <= amount) {
    ...
    entry fun create_hero<CoinType> (
        ...
        let token_data_id = token::create_tokendata(
                ...
                // type
                vector<String>[string::utf8(BURNABLE_BY_OWNER),string::utf8(TOKEN_PROPERTY_MUTABLE), string::utf8(GAME_STRENGTH), string::utf8(IS_WOLF), string::utf8(IS_HERO), string::utf8(IS_EQUIP)],  // property_keys                
                vector<vector<u8>>[bcs::to_bytes<bool>(&true),bcs::to_bytes<bool>(&true), bcs::to_bytes<u64>(&init_str), bcs::to_bytes<bool>(&is_wolf), bcs::to_bytes<bool>(&true), bcs::to_bytes<bool>(&false)],  // values 
                vector<String>[string::utf8(b"bool"),string::utf8(b"bool"), string::utf8(b"u64"), string::utf8(b"bool"), string::utf8(b"bool"), string::utf8(b"bool")],
        );
    ...
    entry fun entering_dungeon<WarCoinType> (
        ...
        if (token_id_1_str < 50) {
            ...
            if(win) {                
                ...
                item_material_drop(sender, game_address, string::utf8(MATERIAL_A), 60);
            } else {
                event::emit_event(&mut game_events.dungeon_result_events, GameResultDungeonEvent {            
                    win: false,
                    battle_time:now_second,
                ...
                item_material_drop(sender, game_address, string::utf8(MATERIAL_G), 20);
            };            
        };
        ...
        if(token_id_1_str < 100 && token_id_1_str >= 50) {
            ...
            if(win) {
                ...
                item_material_drop(sender,game_address, string::utf8(MATERIAL_B), 60);
            } else {
                event::emit_event(&mut game_events.dungeon_result_events, GameResultDungeonEvent {            
                    win: false,
                    battle_time:now_second,
        ...
        if(token_id_1_str >= 100 && token_id_1_str <= 400) {
            ...
            if(win) {
                ...
                item_material_drop(sender, game_address, string::utf8(MATERIAL_C), 60);
            } else {
                item_material_drop(sender, game_address, string::utf8(MATERIAL_F), 10);
                event::emit_event(&mut game_events.dungeon_result_events, GameResultDungeonEvent {            
                    win: false,
                    battle_time:now_second,
                    earn:0,
        ...
        if(token_id_1_str > 400 && token_id_1_str <= 600) {
            ...
            if(win) {
                ...
                item_material_drop(sender,game_address, string::utf8(MATERIAL_D), 60);
            } else {
                event::emit_event(&mut game_events.dungeon_result_events, GameResultDungeonEvent {            
                    win: false,
                    battle_time:now_second,
        ...
        if(token_id_1_str > 600) {
            ...
            if(win) {
                ...
                item_material_drop(sender,game_address, string::utf8(MATERIAL_E), 30);
            } else {
                event::emit_event(&mut game_events.dungeon_result_events, GameResultDungeonEvent {            
                    win: false,
                    battle_time:now_second,
    ...
    entry fun entering_personal_dungeon<WarCoinType> (
        ...
        if (token_id_1_str >= 100) {            
            ...
            if(win) {                
                ...
                if(table::contains(&mut personal_regen_timer.timer_2, token_id_1)) {
    ...
    entry fun exploration<WarCoinType> (
        ...
        if (token_id_1_str < 50) {            
            ...
            if(win) {
                if(random < 5) {
                    ...
                    item_material_drop(sender,game_address, string::utf8(MATERIAL_G), 30);
                };
        ...
        if(token_id_1_str >= 50) {            
            ...
            if(win) {
                if(random < 5) {
                    item_material_drop(sender,game_address, string::utf8(MATERIAL_J), 30);
                } else {
                    item_material_drop(sender,game_address, string::utf8(MATERIAL_I), 30);
                };                
